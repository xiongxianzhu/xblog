...
<p align="center">
  ...
  <sub>FastAPI 后端 · Next.js 前端 · Markdown 写作 · MIT 开源</sub>
</p>
...
> 💡 后端端口非 `8000` 时，在 `frontend/.env.local` 设置 `BACKEND_URL`。
...
```text
① 配置 backend/.env + frontend/.env.local（含 REVALIDATE_SECRET）
...
| 前端 API 404 | 后端未启动 / `BACKEND_URL` 错误 | `make dev` + 检查 `.env.local` |
